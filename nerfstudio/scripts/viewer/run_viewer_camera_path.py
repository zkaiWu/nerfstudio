...
@dataclass
class RunViewerCameraPath:
    """Load a checkpoint and start the viewer."""

    load_config: Path
    """Path to config YAML file."""
    camera_path: Path
    """camera path to draw the camera pose"""
    ...
    def main(self) -> None:
        ...
        _start_viewer(config, pipeline, step, self.camera_path)
...
def _start_viewer(config: TrainerConfig, pipeline: Pipeline, step: int, camera_path:Path):
    """Starts the viewer
    ...
    with open(camera_path, "r") as f: 
        camera_data = json.load(f)['camera_path']
    camera_to_worlds = [torch.tensor(meta_data['camera_to_world']).view(4, 4)[:, :3] for meta_data in camera_data]
    camera_to_worlds = torch.stack(camera_to_worlds, dim=0)
    pipeline.datamanager.train_dataset.cameras.camera_to_worlds = camera_to_worlds
    viewer_state.init_scene_empty_image(
...
def entrypoint():
    ...
    tyro.cli(RunViewerCameraPath).main()
...
# For sphinx docs
get_parser_fn = lambda: tyro.extras.get_parser(RunViewerCameraPath)  # noqa
